		...
		return (
			<div key={`${herb.id}-${index}`} className="cart-container">
				<img
					...
					alt={herb.alt}
					className="cart-img profile"
				/>
				<div>
					{herb.quantity > 1 ? (
						<div>
							{herb.quantity} {herb.name}s
						</div>
					) : (
						<div>
							{herb.quantity} {herb.name}
						</div>
					)}
					<div>
						sub-total: <span className="bold-text">{herb.price} €</span>
					</div>

					<div className="cart-buttons-container">
						<button type="button" onClick={() => addToCart(herb)}>
							Add
						</button>
						<button type="button" onClick={() => removeFromCart(herb)}>
							Remove
						</button>
					</div>
				</div>
	...
	return (
		<div>
			<div className="checkout-nav">
				<Link to="/" className="to-btn">
					Back to Home
				</Link>
				<Link to="/products" className="to-btn">
					Back to Products
				</Link>
			</div>
